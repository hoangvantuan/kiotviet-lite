...
import { bigint, check, index, pgTable, timestamp, unique, uuid } from 'drizzle-orm/pg-core'
...
export const volumePrices = pgTable(
  ...
  (table) => [
    ...
    check('check_volume_prices_min_qty_positive', sql`${table.minQty} > 0`),
  ],
)
