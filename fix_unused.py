...
def fix_imports(filepath):
    ...
    # Check if formatVnd is used anywhere else than in import
    if 'formatVnd' in content:
        if len(re.findall(r'formatVnd\(', content)) == 0:
            ...
            content = re.sub(r'import\s*\{\s*formatVnd\s*\}\s*from\s*[\'"]@kiotviet-lite/shared[\'"]', '', content)
    ...
    if 'formatVndWithSuffix' in content:
        if len(re.findall(r'formatVndWithSuffix\(', content)) == 0:
            ...
            content = re.sub(r'import\s*\{\s*formatVndWithSuffix\s*\}\s*from\s*[\'"]@kiotviet-lite/shared[\'"]', '', content)
    ...
    if 'AlertCircle' in content:
        if len(re.findall(r'<AlertCircle', content)) == 0 and len(re.findall(r'icon=\{AlertCircle\}', content)) == 0:
            ...
            content = re.sub(r'import\s*\{\s*AlertCircle\s*\}\s*from\s*[\'"]lucide-react[\'"]', '', content)
