...
-- chú. (TIEN-105: hai cột đếm customers.total_purchased, purchase_count ngừng đọc ghi nhưng chưa xóa
-- ở migration này, để API cũ chạy song song lúc triển khai không lỗi; xóa ở bản sau.)
...
-- theo loại dòng, reference_id NULL (bất biến I10 chỉ kiểm dòng có reference_id). Dòng 'purchase'
-- không khớp mã phiếu nào để manual: nhập tay (POST /products/:id/inventory/purchase) cũng ghi loại
-- 'purchase' với ghi chú tự do, không phân biệt được với dòng phiếu nhập mất ghi chú.
