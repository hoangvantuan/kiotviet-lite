...
function workbook(
  ...
  rows: unknown[][],
  headers: readonly string[] = BULK_EXPORT_HEADERS[kind],
) {
  const book = XLSX.utils.book_new()
  XLSX.utils.book_append_sheet(book, XLSX.utils.aoa_to_sheet([[...headers], ...rows]), 'Dữ liệu')
  XLSX.utils.book_append_sheet(
    book,
    XLSX.utils.aoa_to_sheet([['Bỏ qua sheet hướng dẫn']]),
    'Hướng dẫn',
  )
...
function upload(
  ...
  return app.request(`/api/v1/bulk-import/${kind}/preview`, {
    method: 'POST',
    body: form,
...
async function payload(
  response: Response,
): Promise<{ data: BulkImportPreview; error: { message: string } }> {
...
describe('bulk import preview over HTTP (PGlite)', () => {
  ...
  it('rejects malformed ZIP, wrong sheet and missing headers without mutating data', async () => {
    ...
    const malformed = await upload(
      app,
      'products',
      new TextEncoder().encode('PK not zip'),
      'upsert',
      auth,
    )
    ...
    const missing = await upload(
      app,
      'customers',
      workbook('customers', [], ['Mã khách hàng']),
      'upsert',
      auth,
    )
    ...
    XLSX.utils.book_append_sheet(
      book,
      XLSX.utils.aoa_to_sheet([[...BULK_EXPORT_HEADERS.products]]),
      'Khác',
    )
    const wrongSheet = await upload(
      app,
      'products',
  ...
  it('collects multiple field errors and duplicate keys at both sheet rows', async () => {
    ...
    expect(
      data.errors
        .filter((error: { row: number; column: string }) => error.column === 'Mã khách hàng')
        .map((error: { row: number }) => error.row),
    ).toEqual([2, 3])
    expect(new Set(data.errors.map((error: { row: number }) => error.row))).toEqual(
      new Set([2, 3, 4]),
    )
    expect(data.errors.some((error: { column: string }) => error.column === 'Hạn mức nợ')).toBe(
      true,
    )
    const missingPrice = await payload(
      await upload(
        app,
        'products',
        workbook('products', [['SP-NOPRICE', 'Ống nhựa']]),
        'create-only',
        env.owner.authHeader.Authorization,
      ),
    )
    expect(missingPrice.data.errors).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          row: 2,
          column: 'Giá bán',
          message: 'Thiếu giá trị cột Giá bán',
        }),
      ]),
    )
  ...
  it('distinguishes create-only conflicts, upsert no-ops, blank retention and explicit nullable clears', async () => {
    ...
    const rows = [
      ['ncc-exist', '', '__XOA__'],
      ['NCC-NEW', 'Nhà cung cấp B', ''],
    ]
    const bytes = workbook('suppliers', rows)
    const forbiddenUpdate = await upload(
      app,
      'suppliers',
      bytes,
      'create-only',
    ...
    expect(conflictData.errors).toEqual(
      expect.arrayContaining([expect.objectContaining({ row: 2, column: 'Mã nhà cung cấp' })]),
    )
    const update = await upload(
      app,
      'suppliers',
      bytes,
      'upsert',
    ...
    const unchanged = await upload(
      app,
      'suppliers',
      workbook('suppliers', [['ncc-exist', '', '']]),
      'upsert',
      env.owner.authHeader.Authorization,
    )
    expect((await payload(unchanged)).data.noOps).toBe(1)
    expect(
      (await env.db.select().from(suppliers).where(eq(suppliers.code, 'NCC-EXIST')))[0]?.phone,
    ).toBe('0901234567')
  })
  ...
  it('preserves exported records as no-ops and ignores product stock changes', async () => {
    ...
    await env.db
      .insert(customers)
      .values({ storeId: env.storeId, code: 'KH-EXIST', name: 'Khách mẫu' })
    ...
    const changedStock = workbook('products', [
      ['SP-EXIST', '', '', '', '', '', '', '', '', '', '', '', '', '', 999],
    ])
    const stock = await upload(
      app,
      'products',
      changedStock,
      'upsert',
    ...
    expect(
      (await env.db.select().from(products).where(eq(products.sku, 'SP-EXIST')))[0]?.currentStock,
    ).toBe(17)
  })
  ...
  it('resolves only tenant-local records and paths; lists missing product category and brand', async () => {
    ...
    await env.db
      .insert(products)
      .values({ storeId: other!.id, sku: 'SP-OTHER', name: 'Khác', sellingPrice: 5 })
    ...
    const bytes = workbook('products', [
      ['SP-OTHER', 'Ống nhựa PVC', '', 'Ống nhựa > PVC', 'Bình Minh', 100],
    ])
    const response = await upload(
      app,
      'products',
  ...
  it('matches existing category, brand and customer group names in exported workbooks', async () => {
    const [parent] = await env.db
      .insert(categories)
      .values({ storeId: env.storeId, name: 'Thiết bị' })
      .returning()
    const [child] = await env.db
      .insert(categories)
      .values({ storeId: env.storeId, name: 'Ống', parentId: parent!.id })
      .returning()
    const [brand] = await env.db
      .insert(brands)
      .values({ storeId: env.storeId, name: 'Nhựa Việt' })
      .returning()
  ...
  it('ignores extra columns but rejects formulas, forbidden clear tokens, and oversized row count', async () => {
    ...
    const extra = workbook(
      'suppliers',
      [['NCC-EXTRA', 'Nhà cung cấp mới', '', '', '', '', '', 'không nhập']],
      headers,
    )
    const accepted = await payload(
      await upload(app, 'suppliers', extra, 'create-only', env.owner.authHeader.Authorization),
    )
    expect(accepted.data.creates).toBe(1)
    const book = XLSX.utils.book_new()
    const sheet = XLSX.utils.aoa_to_sheet([
      headers,
      ['NCC-FORMULA', 'Nhà cung cấp mới', '', '', '', '', '', ''],
    ])
    ...
    const invalid = await payload(
      await upload(app, 'suppliers', formula, 'upsert', env.owner.authHeader.Authorization),
    )
    expect(invalid.data.errors).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ row: 2, column: 'Tên nhà cung cấp' }),
        expect.objectContaining({ row: 2, column: 'Email' }),
      ]),
    )
    const tooMany = workbook(
      'suppliers',
      Array.from({ length: 12001 }, (_, index) => [`LIMIT-${index}`, `Nhà cung cấp ${index}`]),
    )
    const response = await upload(
      app,
      'suppliers',
  ...
  it('permits only the owner, rejects invalid mode and oversize content before parsing', async () => {
    ...
    expect(
      (await upload(app, 'suppliers', bytes, 'merge', env.owner.authHeader.Authorization)).status,
    ).toBe(400)
    const oversized = await app.request('/api/v1/bulk-import/suppliers/preview', {
      method: 'POST',
      headers: {
  ...
  it('previews 11,055 rows within the request budget and reports measured elapsed time', async () => {
    const rows = Array.from({ length: 11055 }, (_, i) => [
    ...
    console.info(
      `bulk import preview benchmark: 11,055 rows, ${bytes.byteLength} XLSX bytes, ${elapsed} ms`,
    )
  }, 90000)
})
